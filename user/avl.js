...
class AVL {
    ...
    obtenerMayor(raiz){
        if(raiz != null){
            if(raiz.right == null){
                let valor =raiz._pelicula;
                return valor;
            }else{
                return this.obtenerMayor(raiz.derecha);
            }
        }
        return null;
    }
    ...
    eliminar(tmp,id){
        if(tmp._pelicula.id_pelicula==id){
            if(tmp.left==null && tmp.right==null){
                ...
                let x=this.obtenerMayor(tmp.left)
                tmp._pelicula=x
                tmp.left=this.eliminar(tmp.left,x.id_pelicula)
            }else{
                tmp=tmp.right
            }
        }else{
            if(tmp._pelicula.id_pelicula<id){
                tmp.right=this.eliminar(tmp.right,id)
            }else{
                tmp.left=this.eliminar(tmp.left,id)
            }
        }
        return tmp
    }
